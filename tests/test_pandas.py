        ...
        with engine_testaccount.connect() as conn:
            ...
            with conn.begin():
                results = conn.execute(
                    sqlalchemy.insert(users).values(name="jack", fullname="Jack Jones")
                )
    ...
    with engine_testaccount_with_numpy.connect() as conn:
        try:
            ...
            with conn.begin():
                ...
                conn.execute(
                    sqlalchemy.text(
                        f"INSERT INTO {db_parameters['name']}(c1) values(:val)"
                    ),
                    {"val": specific_date},
                )
            ...
            conn.execute(
                sqlalchemy.text(f"DROP TABLE IF EXISTS {db_parameters['name']}")
            )
...
def test_to_sql(engine_testaccount_with_numpy, db_parameters):
    with engine_testaccount_with_numpy.connect() as conn:
        ...
        conn.execute(sqlalchemy.text("create or replace table dst(c1 float)"))
...
def test_timezone(db_parameters, engine_testaccount, engine_testaccount_with_numpy):
    ...
    with sa_engine.connect() as conn:
        ...
        try:
            ...
            conn.execute(sqlalchemy.text(f"DROP TABLE {test_table_name};"))
    ...
    try:
        with engine_testaccount.connect() as conn:
            ...
            with conn.begin():
                ...
                # inserts data with the given id
                conn.execute(
                    users.insert().values({"id": 2, "name": "wendy", "fullname": "Wendy Williams"}),
                )
...
def test_numpy_datatypes(engine_testaccount_with_numpy, db_parameters):
    with engine_testaccount_with_numpy.connect() as conn:
        try:
            ...
            with conn.begin():
                conn.execute(
                    text(f"CREATE OR REPLACE TABLE {db_parameters['name']}(c1 timestamp_ntz)")
                )
                conn.execute(
                    text(f"INSERT INTO {db_parameters['name']}(c1) values(:val)"),
                    {"val": specific_date},
                )
                df = pd.read_sql_query(
                    text(f"SELECT * FROM {db_parameters['name']}"), conn
            ...
            conn.execute(text(f"DROP TABLE IF EXISTS {db_parameters['name']}"))
            engine_testaccount_with_numpy.dispose()
...
def test_to_sql(engine_testaccount_with_numpy, db_parameters):
    with engine_testaccount_with_numpy.connect() as conn:
        ...
        conn.execute(text("create or replace table dst(c1 float)"))
        tbl = pd.read_sql_query(text("select * from src"), conn)
...
def test_percent_signs(engine_testaccount):
    ...
    with engine_testaccount.connect() as conn:
        with conn.begin():
            conn.execute(
                text(f"CREATE OR REPLACE TEMP TABLE {table_name}(c1 int, c2 string)")
            )
            conn.execute(
                text(
                    f"""
